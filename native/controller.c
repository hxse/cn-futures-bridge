...
static BOOL CALLBACK locate(HWND w,LPARAM unused){
    ...
    if(IsWindowVisible(w)&&(wcsstr(title,L"快期2-CTP-上期技术-")||!wcscmp(title,L"用户登录"))){
...
int wmain(int argc,WCHAR **argv){
    ...
    while(fgets(line,sizeof(line),stdin)){
        ...
        DWORD previous_thread=target_thread;main_window=NULL;EnumWindows(locate,0);
        ...
        DWORD_PTR result;BOOL sent=SendMessageTimeoutW(window,msg,0,0,SMTO_ABORTIFHUNG,timeout,&result);
