...
int wmain(int argc,WCHAR **argv){
    ...
    while(fgets(line,sizeof(line),stdin)){
        ...
        do{
            ...
            if(main_window||action!=WINDOWS||!s->startup_active||GetTickCount64()>=s->startup_until)break;
        ...
        if(action==WINDOWS&&s->startup_active&&s->startup_until>now+call_timeout)
        ...
        printf("\",\"startup_privacy_count\":%lu,\"startup_terms_count\":%lu,\"startup_wizard_count\":%lu,\"data\":%s}\n",
               s->startup_privacy_count,s->startup_terms_count,s->startup_wizard_count,s->output[0]&&!s->error?s->output:"{}");fflush(stdout);
